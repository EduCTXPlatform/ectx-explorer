...
angular.module('ark_explorer')
  ...
  .filter('currency', function (numberFilter, liskFilter) {
      return function (amount, currency, decimal_places) {
        ...
        if (currency.tickers && currency.tickers.ECTX && currency.tickers.ECTX[currency.symbol]) {
          factor = currency.tickers.ECTX[currency.symbol];
        } else if (currency.symbol !== 'ECTX') {
